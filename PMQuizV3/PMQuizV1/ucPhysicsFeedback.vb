Public Class ucPhysicsFeedback

    Dim ucLoad As MainForm

    Private Sub ucPhysicsFeedback_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtScore = runSQL("SELECT * from PResults where SUserN = '" & Details.LoginUser & "'")
        ...
        If dtScore.Rows.Count > 0 Then 'If there is atleeast a row in the datatable
            runSQL("DELETE * FROM PResults where SUserN = '" & Details.LoginUser & "'")
            'Delete the row
            runSQL("INSERT into PResults (SUserN, Results, TID) values ('" & Details.LoginUser & "','" &
