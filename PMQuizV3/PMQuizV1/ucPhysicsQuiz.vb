Public Class ucPhysicsQuiz

    Dim ucLoad As MainForm

    Private Sub BackBtn_Click(sender As Object, e As EventArgs) Handles BackBtn.Click

        If MsgBox("Are you sure you want to leave, your quiz score won't be saved? ", MsgBoxStyle.YesNo, "Physics Quiz") = MsgBoxResult.Yes Then
    ...
    Private Sub ucPhysicsQuiz_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        Dim dtLdQtn As DataTable 'Assigns the datatable to a variable
        Dim RowAmount, ranNum As Integer 'Creates 2 integer variables

        dtLdQtn = runSQL("Select * from PQuestions") 'Loads the PQuestion field from the PQuestions table in the access database
    ...
    Private Sub AnsBtn_Click(sender As Object, e As EventArgs) Handles AnsBtn.Click
        ...
        If Rbtn1.Checked Then 'If the 1st radio button is ticked then
            dtLdAns = runSQL("Select * from PQuestions where PA1 = '" & Rbtn1.Text & "'") 'Loads from the database the rows which have the PA1 value equal to the radiobutton text
        ...
        If RBtn2.Checked Then 'Same but with the 2nd radio button
            dtLdAns = runSQL("Select * from PQuestions where PA1 = '" & RBtn2.Text & "'")
        ...
        If RBtn3.Checked Then 'Same but with the 3rd radio button
            dtLdAns = runSQL("Select * from PQuestions where PA1 = '" & RBtn3.Text & "'")
            If dtLdAns.Rows.Count > 0 Then
                MsgBox("                         CORRECT! 
        ...
        If RBtn4.Checked Then 'Same but with the 4th radio button
            dtLdAns = runSQL("Select * from PQuestions where PA1 = '" & RBtn4.Text & "'")
            If dtLdAns.Rows.Count > 0 Then
                MsgBox("                         CORRECT! 
    ...
    Private Sub FeedBackLd()
        If Details.QtnTotal = 21 Then
            ucLoad = Me.Parent 'Sets ucLoad to the parent of main form which allows ucLoad to access AddUC
            ucLoad.addUC("ucPhysicsFeedback") 'Puts this string into the subroutine AddUC which determines which user control to load
            Me.Dispose() 'Gets rid of the user control
            GC.Collect() 'Gather all the remaining data left after closing the user control
        End If
    End Sub
